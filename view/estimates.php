...
<?php } ?>
<div class = 'row estimates'>
    <div class = 'col'></div>
    <div class = 'col-10'>
        <form method = "post">
            <div class = 'add-btn'>
                <input type = 'submit' class = 'btn btn-success' value = 'Добавить оценку' name = 'add'>
            </div>
            <table class = 'table table-striped table-bordered'>
                ...
                <?php $count = 0; foreach ($pageData as $key => $value) { $count++; ?>
                    <tr>
                        <td><?= $count ?></td>
                        <?php foreach ($value as $key2 => $value2) { if (stripos($key2, "id") === false) { ?>
                            <td><?= $value2 ?></td>
                        <?php } } ?>
